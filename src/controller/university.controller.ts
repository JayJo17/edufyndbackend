...
export let deleteUniversity = async (req, res, next) => {
    try {
        const universityDetails = await University.findOneAndDelete({ _id: req.query._id }, {
            $set: {
                isDeleted: true,
        ...
        response(req, res, activity, 'Level-2', 'Delete-University', true, 200, universityDetails, clientError.success.deleteSuccess);
    }
    catch (err: any) {
